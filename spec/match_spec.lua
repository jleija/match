...
describe("match", function()
    ...
    describe("variable capture", function()
        ...
        it("can collect and return multiple matched variables for simpler extraction of values", function()
            local y, x = m.collect({x=m.var(2), y=m.var(1)}, {{a=8,x=3,y=4}})
            assert.is.equal(4, y)
            assert.is.equal(3, x)
        end)
        it("can collect and return a table of matched variables for simpler extraction of values", function()
            local vars = m.collect({x=m.var'x', y=m.var'y'}, {{a=8,x=3,y=4}})
